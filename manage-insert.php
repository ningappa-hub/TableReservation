...
$con = connect();
	if (isset($_POST['regascus'])){
        ...
        // Define $location, e.g., set a default value or retrieve from POST data
        $location = isset($_POST['location']) ? intval($_POST['location']) : 0; // Adjust as needed
				 ...
				 // files handle
				    ...
				    if ($extension =="jpg" || $extension =="png" || $extension =="jpeg"){
				    	...
				    	$iquery="INSERT INTO `restaurant_info`(`restaurent_name`, `email`, `phone`, `address`, `location`, `logo`, `password`, `role`) 
			        		VALUES ('$fullname','$email','$phone','$address','$location','$file_name','$password','$role');";
        	...
        	echo '<script>window.location="register.php"</script>';
	        	...
	        	if (isset($_FILES['image'])) {
				 // files handle
				    ...
				    // get the file in temporary
				    $file_tmp = $_FILES['image']['tmp_name'];
				    // get the file extension, pathinfo($file_name,PATHINFO_EXTENSION);
    ...
    if (isset($_POST['book'])) {
        ...
        if ($cinsert == true) {
            ...
            $chairs = []; // Populate this array based on your application's logic
            $menus = [];  // Populate this array based on your application's logic
            
            foreach ($cbbookresult as $r2) {
                $chairs[] = [
                    'chair_id' => $r2['chair_id'],
                    'chair_no' => $r2['chair_no']
                ];
            }
            ...
            $chairs_json = json_encode($chairs);
            ...
            if ($stmt->execute()) {
                echo '<script>alert("Booking and related details successfully inserted.")</script>';
